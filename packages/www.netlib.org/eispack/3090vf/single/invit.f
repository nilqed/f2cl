...
c
      integer i,j,k,l,m,n,s,ip,mm,mp,nm,ns,n1,uk,its,ierr
...
c
         do 780 i = 1, uk
            ...
            if (ilambd .ne. 0.0e0) then
c--               x = pythag(rv1(i),rv2(i))
               if (abs(rv1(i)).gt.abs(rv2(i))) then
                  x = abs(rv1(i))*sqrt(1e0+(rv2(i)/rv1(i))**2)
               else if (rv2(i).ne.0e0) then
                  x = abs(rv2(i))*sqrt((rv1(i)/rv2(i))**2+1e0)
               else
                  x = abs(rv1(i))
               endif
            endif
