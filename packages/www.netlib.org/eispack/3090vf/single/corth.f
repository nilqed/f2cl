...
c
         g = sqrt(h)
c--         f = pythag(ortr(m),orti(m))
         if (abs(ortr(m)).gt.abs(orti(m))) then
            f = abs(ortr(m))*sqrt(1e0+(orti(m)/ortr(m))**2)
         else if (orti(m).ne.0e0) then
            f = abs(orti(m))*sqrt((ortr(m)/orti(m))**2+1e0)
         else
            f = abs(ortr(m))
         endif
