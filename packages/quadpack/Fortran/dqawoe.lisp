    ...
    (prog ((rlist2 (make-array 52 :element-type 'double-float))
      ...
      (declare (type (array double-float (52)) rlist2)
               ...
               (type (double-float) width uflow small reseps resabs oflow
                ...
                epmach dres domega defabs defab2 defab1 correc b2 b1 a2 a1
      ...
      (setf (f2cl-lib:fref rlist-%data% (1) ((1 limit)) rlist-%offset%) 0.0d0)
      (setf (f2cl-lib:fref elist-%data% (1) ((1 limit)) elist-%offset%) 0.0d0)
     ...
     label5
      ...
      (if (> (* 0.5d0 (f2cl-lib:dabs (- b a)) domega) 2.0d0)
          (go label10))
      (setf numrl2 1)
     ...
     label10
      (if (<= (* 0.25d0 (f2cl-lib:dabs (- b a)) domega) 2.0d0)
          (setf extall f2cl-lib:%true%))
      (setf ksgn -1)
      (if (>= dres (* (- 1.0d0 (* 50.0d0 epmach)) defabs))
          (setf ksgn 1))
      (f2cl-lib:fdo (last$ 2 (f2cl-lib:int-add last$ 1))
        ...
        (tagbody
         ...
         label90
          ...
          (if (and (> ktmin 5) (< abserr (* 0.001d0 errsum)))
              (setf ier 5))
          (if (>= abseps abserr)
              (go label100))
          (setf ktmin 0)
     ...
     label150
      ...
      (if (= ier 0)
          (setf ier 3))
      (if (and (/= result 0.0d0) (/= area 0.0d0))
          (go label160))
      (if (> abserr errsum)
          (go label170))
      (if (= area 0.0d0)
     ...
     label165
      (if (and (= ksgn -1)
               (<= (f2cl-lib:dmax1 (f2cl-lib:dabs result) (f2cl-lib:dabs area))
                   (* defabs 0.01d0)))
          (go label190))
      (if (or (> 0.01d0 (/ result area)) (> (/ result area) 100.0d0)
